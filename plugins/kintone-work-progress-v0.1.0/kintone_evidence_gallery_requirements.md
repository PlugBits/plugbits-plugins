...
- **詳細画面で Ctrl/⌘+V の「貼り付け一発」**でスクショを進捗ファイルとして保存
...
- 画像の **ペースト / ドラッグ&ドロップ / ファイル選択** で進捗ファイルを追加
...
- バックエンド：サブテーブル `進捗ファイル`（`画像` 添付 / `メモ` 文字列）
- フロント：詳細画面にギャラリーを描画。**標準サブテーブルUIはCSSで非表示**。
- 利点：セットアップ最短。既存アプリを変更最小。
- 欠点：別用途へ横断集計する場合はAPI経由が前提。

### B）子アプリ分離（Progress Files アプリ）
- バックエンド：子アプリ `Progress Files`（`親レコードID`、`画像`、`メモ`、`作成者`、`作成日時`）
...
## 14. 将来拡張（Optional）
- 子アプリ（Progress Files）への切替（データアクセス層差し替え）
