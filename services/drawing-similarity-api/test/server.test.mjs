...
// OCR・AI・埋め込みを一切呼ばず、既存ポイントのpayloadにthumb_enc/thumb_verだけを
// set_payloadで追記する軽量経路。フル再登録（/index）の代替ではない。

test('thumb-backfill: 既存ポイントにthumb_encをset_payloadし、既存payloadフィールドは保持される', async () => {
  const thumbKey = randomBytes(32);
  // 遡及対象: サムネイルキー無しで先に登録した既存レコード
  const indexRes = await postJson(api.url, '/index', {
    appId: '1', recordId: 'backfill-1', tenantId: 'tenant-backfill',
    drawingNo: 'DWG-BACKFILL-001', productName: 'part-backfill', fileKey: 'file-a', fileName: 'a.pdf'
  });
  assert.equal(indexRes.status, 202, await indexRes.text());
  const before = [...mock.state.points.values()].find((p) => p.payload.record_id === 'backfill-1');
  assert.ok(before, '遡及前提のポイントが存在する');
  assert.equal(before.payload.thumb_enc, undefined, '遡及前はthumb_encが無い');

  const res = await fetch(api.url + '/thumb-backfill', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/octet-stream',
      'X-Index-Meta': encodeURIComponent(JSON.stringify({
        tenantId: 'tenant-backfill', recordId: 'backfill-1', thumbKey: thumbKey.toString('base64')
      }))
    },
    body: PDF_A
  });
  ...
  const data = JSON.parse(text);
  assert.equal(data.ok, true);
  assert.equal(data.recordId, 'backfill-1');

  const after = [...mock.state.points.values()].find((p) => p.payload.record_id === 'backfill-1');
  assert.ok(after.payload.thumb_enc, 'thumb_encが追加される');
  assert.equal(after.payload.thumb_ver, 1);
  // 既存フィールドは維持される（set_payloadはベクトル・他フィールドを変更しない）
  assert.equal(after.payload.drawing_no, 'DWG-BACKFILL-001', '既存のOCR系フィールドは維持される');
  assert.equal(after.payload.product_name, 'part-backfill');
  assert.equal(after.payload.file_key, 'file-a');

  // 復号ラウンドトリップ確認
  const raw = Buffer.from(after.payload.thumb_enc, 'base64');
  const iv = raw.subarray(0, 12);
  const authTag = raw.subarray(raw.length - 16);
  const ciphertext = raw.subarray(12, raw.length - 16);
  const decipher = createDecipheriv('aes-256-gcm', thumbKey, iv);
...
test('thumb-backfill: OCR・AI・埋め込みは呼ばない（/indexのようなpoints upsertは発生しない）', async () => {
  const upsertsBefore = mock.state.captured.upserts.length;
  const thumbKey = randomBytes(32);
  const res = await fetch(api.url + '/thumb-backfill', {
    method: 'POST',
    headers: {
  ...
  assert.equal(res.status, 200, await res.text());
  assert.equal(mock.state.captured.upserts.length, upsertsBefore, 'points(PUT)によるupsertは発生しない（set_payloadのみ）');
});

test('thumb-backfill: recordIdが無ければ400', async () => {
  const res = await fetch(api.url + '/thumb-backfill', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/octet-stream',
      'X-Index-Meta': encodeURIComponent(JSON.stringify({
        tenantId: 'tenant-backfill', thumbKey: randomBytes(32).toString('base64')
      }))
    },
    body: PDF_A
  });
...
test('thumb-backfill: thumbKeyが不正（32バイトでない）なら400', async () => {
  const res = await fetch(api.url + '/thumb-backfill', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/octet-stream',
      'X-Index-Meta': encodeURIComponent(JSON.stringify({
        tenantId: 'tenant-backfill', recordId: 'backfill-1', thumbKey: 'not-32-bytes'
      }))
    },
    body: PDF_A
  });
  ...
  const data = JSON.parse(text);
  assert.match(data.error, /thumbKey/);
});

test('thumb-backfill: 未登録のrecordIdは404', async () => {
  const res = await fetch(api.url + '/thumb-backfill', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/octet-stream',
      'X-Index-Meta': encodeURIComponent(JSON.stringify({
        tenantId: 'tenant-backfill', recordId: 'backfill-nonexistent', thumbKey: randomBytes(32).toString('base64')
      }))
    },
    body: PDF_A
  });
...
test('thumb-backfill: x-index-meta が壊れていれば400', async () => {
  const res = await fetch(api.url + '/thumb-backfill', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/octet-stream',
...
test('thumb-backfill: Content-Type が application/octet-stream でなければ400', async () => {
  const res = await postJson(api.url, '/thumb-backfill', {
    tenantId: 'tenant-backfill', recordId: 'backfill-1', thumbKey: randomBytes(32).toString('base64')
  });
  assert.equal(res.status, 400);
});

test('index-status: hasThumbはthumb_encを保存済みの点でtrue、未保存の点でfalse', async () => {
  const res = await fetch(api.url + '/index-status?tenantId=tenant-thumb');
  assert.equal(res.status, 200);
  const data = await res.json();
  const thumb1 = data.items.find((it) => it.recordId === 'thumb-1');
  const thumb2 = data.items.find((it) => it.recordId === 'thumb-2');
  assert.ok(thumb1, 'thumb-1がitemsに含まれる');
  assert.equal(thumb1.hasThumb, true, 'thumb_encを保存済みのためhasThumb=true');
  assert.ok(thumb2, 'thumb-2がitemsに含まれる');
  assert.equal(thumb2.hasThumb, false, 'thumb_enc未保存のためhasThumb=false');
});
...
test('auth: thumb-backfill はキーのテナントに強制され、他テナントrecordIdへのbackfillは404で失敗する', async () => {
  // key-a（tenant-a）で先に登録
  const indexRes = await postJson(authApi.url, '/index', {
    appId: '1', recordId: 'backfill-auth-1', tenantId: '無視される値',
    drawingNo: 'DWG-BACKFILL-AUTH-1', fileKey: 'file-a', fileName: 'a.pdf'
  }, { 'X-API-Key': 'key-a' });
  assert.equal(indexRes.status, 202, await indexRes.text());

  // legacy-key（tenant-b強制）で同じ recordId への遡及を試みても、テナント名前空間が
  // 異なるポイントID（sha256(tenant-b:backfill-auth-1)）になるため見つからず404になる
  const crossRes = await fetch(authApi.url + '/thumb-backfill', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/octet-stream',
      'X-Index-Meta': encodeURIComponent(JSON.stringify({
        recordId: 'backfill-auth-1', thumbKey: randomBytes(32).toString('base64')
      })),
      'X-API-Key': 'legacy-key'
    },
    body: PDF_A
  });
  assert.equal(crossRes.status, 404, await crossRes.text());
  const crossStored = [...authMock.state.points.values()].find((p) => p.payload.record_id === 'backfill-auth-1');
  assert.equal(crossStored.payload.thumb_enc, undefined, '他テナントキーでの試行ではthumb_encが付かない');

  // 正しいテナント（key-a）なら成功する
  const okRes = await fetch(authApi.url + '/thumb-backfill', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/octet-stream',
      'X-Index-Meta': encodeURIComponent(JSON.stringify({
  ...
  const stored = [...authMock.state.points.values()].find((p) => p.payload.record_id === 'backfill-auth-1');
  assert.ok(stored.payload.thumb_enc, '正しいテナントのキーではthumb_encが保存される');
});
